...
router.post('/', async (req, res) => {
    try {
        const { name, email, address, date, instrumentCat, instrumentBrand,brand, warranty,issueType,price, issueDescription, status } = req.body;

        if (!name || !email || !address || !date || !instrumentCat || !instrumentBrand || !brand|| !warranty||!issueType|| !issueDescription|| !status|| !price) {
